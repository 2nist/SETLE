...
juce::String modelQualityForType(const juce::String& type)
{
    if (type == "maj") return "major";
    if (type == "min") return "minor";
    if (type == "dim") return "diminished";
    return type;
}

juce::String symbolForType(const juce::String& root, const juce::String& type)
{
...
setle::model::Chord buildChordForDegree(const juce::String& keyRoot,
                                        const juce::String& mode,
                                        int degreeIndex)
{
    const auto info = buildDiatonicChord(keyRoot, mode, degreeIndex);
    const auto rootMidi = 60 + pitchClassForRoot(info.root);
    auto chord = setle::model::Chord::create(symbolForType(info.root, info.type),
                                             modelQualityForType(info.type),
                                             rootMidi);
    chord.setName(chord.getSymbol());
    chord.setFunction(info.function);
    return chord;
}

setle::model::Chord buildChordForCenter(const juce::String& keyRoot,
                                        const juce::String& mode,
...
std::vector<setle::model::Chord> buildProgressionFromTemplate(const juce::String& keyRoot,
                                                              const juce::String& mode,
...
setle::model::Chord tritoneSub(const setle::model::Chord& dominant)
{
    const int substitutedRoot = juce::jlimit(0, 127, dominant.getRootMidi() + 6);
    auto result = setle::model::Chord::create(rootNameForPitchClass(substitutedRoot) + "7",
                                              "dominant7",
                                              substitutedRoot);
    result.setDurationBeats(dominant.getDurationBeats());
    return result;
}

setle::model::Chord neapolitan(const juce::String& keyRoot)
{
    const int rootMidi = 60 + pitchClassForRoot(keyRoot) - 1;
    auto result = setle::model::Chord::create(rootNameForPitchClass(rootMidi) + "maj",
                                              "major",
                                              juce::jlimit(0, 127, rootMidi));
    result.setDurationBeats(2.0);
    return result;
}
}
